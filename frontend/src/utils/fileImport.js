...
function xmlToJson(xmlDoc) {
    ...
    let stat = evaluateXPath(xmlDoc, 'biotronik-ieee11073-export/dataset/section[@name="MDC"]/section[@name="IDC"]/section[@name="STAT"]//value');
    ...
    let msmtCap = evaluateXPath(xmlDoc, 'biotronik-ieee11073-export/dataset/section[@name="MDC"]/section[@name="IDC"]/section[@name="MSMT"]//section[@name="CAP"]//value');
    ...
    let msmtHv = evaluateXPath(xmlDoc, 'biotronik-ieee11073-export/dataset/section[@name="MDC"]/section[@name="IDC"]/section[@name="MSMT"]//section[@name="LEADHVCHNL"]//value');
    let msmtLv = evaluateXPath(xmlDoc, 'biotronik-ieee11073-export/dataset/section[@name="MDC"]/section[@name="IDC"]/section[@name="MSMT"]//section[@name="LEADCHNL_LV"]//value');
...
function updateXmlDataResult(parsedData) {
    ...
    dataResult.mdc_idc_set_brady_max_tracking_rate = parsedData.brady.MAX_TRACKING_RATE;
    dataResult.mdc_idc_set_brady_max_sensor_rate = parsedData.brady.MAX_SENSOR_RATE;
    ...
    // Patient statistics
    dataResult.mdc_idc_stat_brady_ra_percent_paced = parsedData.stat.RA_PERCENT_PACED;
    dataResult.mdc_idc_stat_brady_rv_percent_paced = parsedData.stat.RV_PERCENT_PACED;
    dataResult.mdc_idc_stat_brady_lv_percent_paced = parsedData.stat.LV_PERCENT_PACED;
...
function updateLogDataResult(parsedData) {
    ...
    if (parsedData["105"] && parsedData["105"].value) {
        dataResult.report_date = new Date(parsedData[105].value).toISOString().split('T')[0];
    }
    //serial
...
function updateBnkDataResult(parsedData, deviceSerial) {
    ...
    if (parsedData["CapformChargeTime"]) {    
        dataResult.mdc_idc_cap_charge_time = parsedData["CapformChargeTime"].slice(0, -2).trim();
    }

    
    if(parsedData["ManualIntrinsicResult.LVMsmt.Msmt"]) {
        dataResult.mdc_idc_msmt_lv_sensing_mean = parsedData["ManualIntrinsicResult.LVMsmt.Msmt"].slice(0, -3).trim();
    }
    if(parsedData["ManualLeadImpedData.LVMsmt.Msmt"]) {
        dataResult.mdc_idc_msmt_lv_impedance_mean = parsedData["ManualLeadImpedData.LVMsmt.Msmt"].slice(0, -5).trim();
    }
    if(parsedData["InterPaceThreshResult.LVMsmt.Amplitude"]) {
        dataResult.mdc_idc_msmt_lv_threshold = parsedData["InterPaceThreshResult.LVMsmt.Amplitude"].slice(0, -3).trim();
    }
    if(parsedData["InterPaceThreshResult.LVMsmt.PulseWidth"]) {
        dataResult.mdc_idc_msmt_lv_pw = parsedData["InterPaceThreshResult.LVMsmt.PulseWidth"].slice(0, -3).trim();
    }


    if(parsedData["ManualIntrinsicResult.RVMsmt.Msmt"]) {
        dataResult.mdc_idc_msmt_rv_sensing_mean = parsedData["ManualIntrinsicResult.RVMsmt.Msmt"].slice(0, -3).trim();
    }
    if(parsedData["ManualLeadImpedData.RVMsmt.Msmt"]) {
        dataResult.mdc_idc_msmt_rv_impedance_mean = parsedData["ManualLeadImpedData.RVMsmt.Msmt"].slice(0, -5).trim();
    }
    if(parsedData["InterPaceThreshResult.RVMsmt.Amplitude"]) {
        dataResult.mdc_idc_msmt_rv_threshold = parsedData["InterPaceThreshResult.RVMsmt.Amplitude"].slice(0, -3).trim();
    }
    if(parsedData["InterPaceThreshResult.RVMsmt.PulseWidth"]) {
        dataResult.mdc_idc_msmt_rv_pw = parsedData["InterPaceThreshResult.RVMsmt.PulseWidth"].slice(0, -3).trim();
    }
    if(parsedData["ShockLastImpedance"]) {
        dataResult.mdc_idc_msmt_shock_impedance = parsedData["ShockLastImpedance"].slice(0, -5).trim();
    }
    

    if(parsedData["ManualIntrinsicResult.RAMsmt.Msmt"]) {
        dataResult.mdc_idc_msmt_ra_sensing_mean = parsedData["ManualIntrinsicResult.RAMsmt.Msmt"].slice(0, -3).trim();
    }
    if(parsedData["ManualLeadImpedData.RAMsmt.Msmt"]) {
        dataResult.mdc_idc_msmt_ra_impedance_mean = parsedData["ManualLeadImpedData.RAMsmt.Msmt"].slice(0, -5).trim();
    }
    if(parsedData["InterPaceThreshResult.RAMsmt.Amplitude"]) {
        dataResult.mdc_idc_msmt_ra_threshold = parsedData["InterPaceThreshResult.RAMsmt.Amplitude"].slice(0, -3).trim();
    }
    if(parsedData["InterPaceThreshResult.RAMsmt.PulseWidth"]) {
        dataResult.mdc_idc_msmt_ra_pw = parsedData["InterPaceThreshResult.RAMsmt.PulseWidth"].slice(0, -3).trim();
    }
